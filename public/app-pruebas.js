...
function initUI() {
    ...
    const fields = [
        { id: 'padecimiento', limit: 400 }, { id: 'receta', limit: 200 },
...
function configurarImpresionReceta() {
    ...
    btnPrint.addEventListener('click', async () => {
        ...
        // 🔑 PARSEO DE TRATAMIENTO DINÁMICO REQUERIDO:
        // Separa de forma limpia el bloque usando las palabras clave MEDICAMENTOS e INDICACIONES
        const medicamentoMatch = tratamientoRaw.match(/MEDICAMENTOS:\s*([\s\S]*?)(?=INDICACIONES:|$)/i);
        const indicacionesMatch = tratamientoRaw.match(/INDICACIONES:\s*([\s\S]*?)$/i);

        const medicamentosVal = medicamentoMatch ? medicamentoMatch[1].trim() : "Según indicaciones.";
        const indicacionesVal = indicacionesMatch ? indicacionesMatch[1].trim() : "Sin indicaciones adicionales.";
            ...
            <div class="rx-box-container" style="margin-bottom: 10px;">
                ...
                <table class="rx-prescription-table" style="width: 100%; border-collapse: collapse;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="white-space: pre-line; padding: 10px; vertical-align: top; color: #333333;">${indicacionesVal}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
